...
Army::Army(std::string img, float x, float y, float radius, float coolDown, float speed, float hp, int id, float shootRadius, bool dio) :
    Role(img, x, y), coolDown(coolDown), speed(speed), id(id), shootRadius(shootRadius),dio(dio) {
...
void Army::Health(float cocodamage) {
    HP += cocodamage;
}
...
void Army::Dio_move(int id) {
    Velocity = speed * Engine::Point(dire[id][0], dire[id][1]);
}